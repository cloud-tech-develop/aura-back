...
func (s *categoryService) Create(ctx context.Context, c *Category) error {
	...
	c.EmpresaID = ctx.Value("empresa_id").(int64) // Assuming empresa_id is set in context
...
func (s *categoryService) List(ctx context.Context, empresaID int64) ([]Category, error) {
	return s.repo.List(ctx, empresaID)
}
...
func (s *categoryService) Update(ctx context.Context, id int64, c *Category) error {
	...
	c.ID = existing.ID
	c.EmpresaID = existing.EmpresaID
...
func (s *brandService) Create(ctx context.Context, b *Brand) error {
	...
	b.EmpresaID = ctx.Value("empresa_id").(int64)
...
func (s *brandService) List(ctx context.Context, empresaID int64) ([]Brand, error) {
	return s.repo.List(ctx, empresaID)
}
...
func (s *brandService) Update(ctx context.Context, id int64, b *Brand) error {
	...
	b.ID = existing.ID
	b.EmpresaID = existing.EmpresaID
...
func (s *productService) Create(ctx context.Context, p *Product) error {
	...
	p.EmpresaID = ctx.Value("empresa_id").(int64)
	p.Status = "ACTIVE"
	p.CurrentStock = 0 // Default stock

	// Check SKU uniqueness
	_, err := s.repo.GetBySKU(ctx, p.SKU, p.EmpresaID)
	if err == nil {
...
func (s *productService) List(ctx context.Context, empresaID int64, filters ListFilters) ([]Product, error) {
	if filters.Page < 1 {
		filters.Page = 1
	}
	if filters.Limit < 1 {
	...
	return s.repo.List(ctx, empresaID, filters)
}
...
func (s *productService) Update(ctx context.Context, id int64, p *Product) error {
	...
	if p.SKU != existing.SKU {
		_, err := s.repo.GetBySKU(ctx, p.SKU, existing.EmpresaID)
		if err == nil {
			return fmt.Errorf("sku %s already exists", p.SKU)
		}
		if err != sql.ErrNoRows {
	...
	p.ID = existing.ID
	p.EmpresaID = existing.EmpresaID
