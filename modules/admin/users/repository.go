...
func (r *repository) Create(ctx context.Context, user *User) error {
	...
	query := fmt.Sprintf(`
		INSERT INTO %susers (enterprise_id, name, email, password_hash, active, created_at, updated_at)
...
func (r *repository) GetByID(ctx context.Context, id int64) (*User, error) {
	...
	query := fmt.Sprintf(`
		...
		FROM %susers WHERE id = $1 AND deleted_at IS NULL`, prefix)
...
func (r *repository) GetByEmail(ctx context.Context, email vo.Email) (*User, error) {
	...
	query := fmt.Sprintf(`
		SELECT id, enterprise_id, name, email, password_hash, active, created_at, updated_at, deleted_at
		FROM %susers WHERE email = $1 AND deleted_at IS NULL`, prefix)
...
func (r *repository) Update(ctx context.Context, user *User) error {
	...
	query := fmt.Sprintf(`
		...
		WHERE id = $6 AND deleted_at IS NULL`, prefix)
...
func (r *repository) UpdateStatus(ctx context.Context, id int64, active bool) error {
	...
	query := fmt.Sprintf(`
		...
		WHERE id = $3 AND deleted_at IS NULL`, prefix)
...
func (r *repository) ListUserRolesByEnterpriseID(ctx context.Context, enterpriseID int64) ([]UserRole, error) {
	...
	query := fmt.Sprintf(`
		...
		FROM %suser_roles ur
		JOIN %susers u ON u.id = ur.user_id
		WHERE u.enterprise_id = $1 AND u.deleted_at IS NULL`, prefix, prefix)
